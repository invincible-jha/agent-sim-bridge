...
```bash
pip install "agent-sim-bridge[agentcore]"
```

### 2. Swap the configuration backend

Replace the default ``Settings`` class with the enterprise version
which reads from your AgentPhysical control plane:

```python
# Before (open-source)
from agent_sim_bridge.core import Settings
...
The AgentPhysical SDK ships with a full set of enterprise plugins that
auto-register when the package is installed:

```python
from agent_sim_bridge.plugins.registry import processor_registry
